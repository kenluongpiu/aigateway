...
export const handler = async (context, parameters, eventType, options) => {
    ...
    if (!credentials?.azureAuthMode && !credentials?.apiKey) {
        return {
            error: new Error('Content Safety credentials must include either apiKey or azureAuthMode'),
            verdict: true,
            data,
        };
    }
    ...
    const { token, error: tokenError } = await getAccessToken(credentials, 'contentSafety', options, options?.env);
    if (tokenError) {
        return {
            error: tokenError,
            verdict: true,
    ...
    if (response) {
        ...
        const hasHarmfulContent = response.categoriesAnalysis?.some((category) => {
