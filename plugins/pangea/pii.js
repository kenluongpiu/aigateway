...
export const handler = async (context, parameters, eventType) => {
    ...
    try {
        ...
        const response = await post(url, request, requestOptions, parameters.timeout);
        const piiDetected = response.result?.count > 0 && response.result.redacted_data
            ? true
            : false;
        let shouldBlock = piiDetected;
        if (piiDetected && redact) {
            setCurrentContentPart(context, eventType, transformedData, response.result.redacted_data);
            shouldBlock = false;
            transformed = true;
        }
        return {
