...
const maskPiiEntries = (text, piiEntries) => {
    return piiEntries.reduce((maskedText, entry) => {
        const maskText = `[${entry.entity_type.toUpperCase()}]`;
        return (maskedText.slice(0, entry.start + 1) +
            maskText +
            maskedText.slice(entry.end));
    }, text);
};
export const redactPii = async (text, timeout) => {
    if (!text) {
    ...
    const result = await postPromptfoo('pii', piiObject, timeout);
    ...
    if (piiResult.flagged) {
        ...
        const sortedPiiEntries = piiResult.payload.pii.sort((a, b) => b.start - a.start);
...
export const handler = async (context, parameters, eventType) => {
    ...
    try {
        ...
        const results = await Promise.all(textArray.map((text) => redactPii(text, parameters.timeout)));
