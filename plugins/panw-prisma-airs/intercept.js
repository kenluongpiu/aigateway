...
const AIRS_URL = 'https://service.api.aisecurity.paloaltonetworks.com/v1/scan/sync/request';
const fetchAIRS = async (payload, apiKey, timeout) => {
    const opts = {
        headers: { 'x-pan-token': apiKey },
    };
    return post(AIRS_URL, payload, opts, timeout);
};
export const handler = async (ctx, params, hook) => {
    const apiKey = params.credentials?.AIRS_API_KEY ||
    ...
    try {
        ...
        const payload = {
            tr_id: typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
                ? crypto.randomUUID()
        ...
        const res = await fetchAIRS(payload, apiKey, params.timeout);
