...
export const handler = async (context, parameters, eventType) => {
    ...
    try {
        ...
        if (!operator || !['any', 'all', 'none'].includes(operator)) {
            throw new Error('Invalid or missing operator (must be "any", "all", or "none")');
        }
        ...
        data = {
            ...
            verdict,
            explanation: getExplanation(operator, bestMatch.presentKeys, bestMatch.missingKeys, verdict),
...
function getExplanation(operator, presentKeys, missingKeys, verdict) {
    const presentKeysList = presentKeys.length > 0
        ? `Found keys: [${presentKeys.join(', ')}]`
        : 'No matching keys found';
    const missingKeysList = missingKeys.length > 0
