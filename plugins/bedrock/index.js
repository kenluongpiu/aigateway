import { getCurrentContentPart, HttpError, setCurrentContentPart, } from '../utils';
...
export const validateCreds = (credentials) => {
    return REQUIRED_CREDENTIAL_KEYS.every((key) => Boolean(credentials?.[key]));
};
export const handleCredentials = async (options, credentials) => {
    const finalCredentials = {};
    if (credentials?.awsAuthType === 'assumedRole') {
        try {
            // Assume the role in the source account
            const sourceRoleCredentials = await getAssumedRoleCredentials(options.getFromCacheByKey, options.putInCacheWithValue, options.env, options.env.AWS_ASSUME_ROLE_SOURCE_ARN, // Role ARN in the source account
            ...
            const destinationCredentials = (await getAssumedRoleCredentials(options.getFromCacheByKey, options.putInCacheWithValue, options.env, credentials.awsRoleArn || '', credentials.awsExternalId || '', credentials.awsRegion || '', {
...
export const pluginHandler = async (context, parameters, eventType, options) => {
    ...
    try {
        ...
        const finalCredentials = await handleCredentials(options, credentials);
        ...
        const redact = parameters?.redact;
        ...
        const results = await Promise.all(textArray.map((text) => text
            ? bedrockPost({ ...finalCredentials, guardrailId, guardrailVersion }, {
                content: [{ text: { text } }],
                source: body.source,
            }, parameters.timeout)
            : null));
        const interventionData = results.find((result) => result && result.action === 'GUARDRAIL_INTERVENED');
        if (interventionData) {
            verdict = false;
        }
        ...
        for (const result of results) {
            ...
            sensitiveInfoKeys.forEach((key) => {
                if (sensitiveInfo[key].length > 0) {
        ...
        if (hasTriggeredPII && redact) {
            const maskedTexts = textArray.map((text, index) => redactPii(text, results[index]));
