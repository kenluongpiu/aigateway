import { getCurrentContentPart, getText, setCurrentContentPart, } from '../utils';
import { PORTKEY_ENDPOINTS, fetchPortkey, } from './globals';
export async function detectPII(textArray, parameters, env) {
    const result = await fetchPortkey(env, PORTKEY_ENDPOINTS.PII, parameters.credentials, {
        input: textArray,
...
export const handler = async (context, parameters, eventType, options) => {
    ...
    try {
        ...
        let filteredCategories = detectedCategories.filter((category) => categoriesToCheck.includes(category));
        const hasPII = filteredCategories.length > 0;
        let shouldBlock = hasPII;
        if (parameters.redact && hasPII) {
            setCurrentContentPart(context, eventType, transformedData, mappedTextArray);
            shouldBlock = false;
            transformed = true;
        }
