...
export const handler = async (context, parameters, eventType, options) => {
    ...
    try {
        ...
        const result = await fetchPortkey(options?.env || {}, PORTKEY_ENDPOINTS.MODERATIONS, parameters.credentials, { input: text }, parameters.timeout);
        const categoriesFlagged = Object.keys(result.results[0].categories).filter((category) => result.results[0].categories[category]);
