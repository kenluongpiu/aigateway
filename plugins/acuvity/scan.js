...
const getRedactionList = (parameters) => {
    const redactions = [];
    let pii_redaction = false;
    let secret_redaction = false;
...
export const postAcuvityScan = async (base_url, apiKey, textArray, eventType, redactions) => {
    const data = {
...
export const handler = async (context, parameters, eventType) => {
    ...
    try {
        ...
        const result = await postAcuvityScan(base_url, token, textArray, eventType, redactionList);
        ...
        for (const extraction of result.extractions) {
            ...
            const currentResults = evaluateAllParameters(extraction, parameters, responseHelper);
...
function evaluateAllParameters(extraction, parameters, responseHelper) {
    const guardTypes = new Set();
    // Check prompt injection
    if (parameters.prompt_injection) {
        const check = responseHelper.evaluate(extraction, GuardName.PROMPT_INJECTION, parameters.prompt_injection_threshold || 0.0);
        if (check.matched) {
    ...
    // Check toxic content
    if (parameters.toxic) {
        const check = responseHelper.evaluate(extraction, GuardName.TOXIC, parameters.toxic_threshold || 0.0);
        if (check.matched) {
    ...
    if (parameters.jail_break) {
        const check = responseHelper.evaluate(extraction, GuardName.JAIL_BREAK, parameters.jail_break_threshold || 0.0);
        if (check.matched) {
    ...
    if (parameters.malicious_url) {
        const check = responseHelper.evaluate(extraction, GuardName.MALICIOUS_URL, parameters.malicious_url_threshold || 0.0);
        if (check.matched) {
    ...
    if (parameters.biased) {
        const check = responseHelper.evaluate(extraction, GuardName.BIASED, parameters.biased_threshold || 0.0);
        if (check.matched) {
    ...
    if (parameters.harmful) {
        const check = responseHelper.evaluate(extraction, GuardName.HARMFUL_CONTENT, parameters.harmful_threshold || 0.0);
        if (check.matched) {
    ...
    if (parameters.language && parameters.language_values) {
        const check = responseHelper.evaluate(extraction, GuardName.LANGUAGE, 0.5, parameters.language_values);
        if (check.matched) {
    ...
    if (parameters.pii && parameters.pii_categories) {
        for (const category of parameters.pii_categories) {
            const check = responseHelper.evaluate(extraction, GuardName.PII_DETECTOR, 0.0, category.toLowerCase());
            if (check.matched) {
    ...
    if (parameters.secrets && parameters.secrets_categories) {
        for (const category of parameters.secrets_categories) {
            const check = responseHelper.evaluate(extraction, GuardName.SECRETS_DETECTOR, 0.0, category.toLowerCase());
            if (check.matched) {
