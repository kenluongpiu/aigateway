...
export function maskEntities(text, positions) {
...
const redactPii = async (text, credentials, timeout) => {
    if (!text)
    ...
    const result = await postPatronus(evaluator, credentials, evaluationBody, timeout);
...
export const handler = async (context, parameters, eventType) => {
    ...
    try {
        ...
        const results = await Promise.all(textArray.map((text) => redactPii(text, parameters.credentials, parameters.timeout)));
        const hasPII = results.some((result) => result?.data?.evaluation_result.pass === false);
        const piiData = results.find((result) => result?.maskedText)?.data ?? results[0]?.data;
        error =
