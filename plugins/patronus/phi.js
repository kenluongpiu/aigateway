...
const redactPhi = async (text, credentials, timeout) => {
    if (!text)
...
export const handler = async (context, parameters, eventType) => {
    ...
    try {
        ...
        const results = await Promise.all(textArray.map((text) => redactPhi(text, parameters.credentials, parameters.timeout)));
        const hasPHI = results.some((result) => result?.data?.evaluation_result?.pass === false);
        let shouldBlock = hasPHI;
        const phiData = results.find((result) => result?.maskedText)?.data ?? results[0]?.data;
        error =
