...
if (!isHeadless &&
     ...
     */
    async function sendWithTimeout(fn, timeoutMs = 200) {
    ...
    app.get('/log/stream', (c) => {
        ...
        return streamSSE(c, async (stream) => {
            const addLogClient = c.get('addLogClient');
            const removeLogClient = c.get('removeLogClient');
            const client = {
                sendLog: (message) => sendWithTimeout(() => stream.writeSSE(message)),
            };
            ...
            try {
                ...
                await sendWithTimeout(() => stream.writeSSE({ event: 'connected', data: clientId }));
                ...
                const heartbeatInterval = setInterval(async () => {
                    ...
                    try {
                        await sendWithTimeout(() => stream.writeSSE({ event: 'heartbeat', data: 'pulse' }));
                    }
                    catch (error) {
