 ...
 */
export const retryRequest = async (url, options, retryCount, statusCodesToRetry, timeout, requestHandler, followProviderRetry) => {
    let lastResponse;
    let lastAttempt;
    ...
    try {
        await retry(async (bail, attempt) => {
            try {
                let response;
                if (timeout) {
                    response = await fetchWithTimeout(url, options, timeout, requestHandler);
                }
                ...
                if (statusCodesToRetry.includes(response.status)) {
                    ...
                    if (response.status === 429 && followProviderRetry) {
                        // get retry header.
                        const retryHeader = POSSIBLE_RETRY_STATUS_HEADERS.find((header) => {
                    ...
                    const errorObj = new Error(await response.clone().text());
                    errorObj.status = response.status;
                    errorObj.headers = Object.fromEntries(response.headers);
                    bail(errorObj);
                    return;
        ...
        if (error instanceof TypeError &&
            ...
            lastResponse = new Response(`Message: ${error.message} Cause: ${error.cause ?? 'NA'} Name: ${error.name}`, {
                status: 500,
            });
        }
        else {
        ...
        console.warn(`Tried ${lastAttempt ?? 1} time(s) but failed. Error: ${JSON.stringify(error)}`);
    }
    return {
