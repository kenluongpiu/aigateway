import { AZURE_OPEN_AI, BEDROCK, CONTENT_TYPES, COHERE, GOOGLE, REQUEST_TIMEOUT_STATUS_CODE, PRECONDITION_CHECK_FAILED_STATUS_CODE, GOOGLE_VERTEX_AI, } from '../globals';
...
function getPayloadFromAWSChunk(chunk) {
...
export async function* readAWSStream(reader, transformFunction, fallbackChunkId, strictOpenAiCompliance, gatewayRequest) {
            ...
            if (buffer.length) {
                ...
                while (buffer.length >= expectedLength && buffer.length !== 0) {
                    ...
                    if (transformFunction) {
                        const transformedChunk = transformFunction(payload, fallbackChunkId, streamState, strictOpenAiCompliance, gatewayRequest);
...
export async function* readStream(reader, splitPattern, transformFunction, isSleepTimeRequired, fallbackChunkId, strictOpenAiCompliance, gatewayRequest) {
    let buffer = '';
    const decoder = new TextDecoder();
    ...
    while (true) {
        ...
        if (done) {
            if (buffer.length > 0) {
                if (transformFunction) {
                    yield transformFunction(buffer, fallbackChunkId, streamState, strictOpenAiCompliance, gatewayRequest);
                }
                else {
                    yield buffer;
                }
        ...
        while (buffer.split(splitPattern).length > 1) {
            ...
            for (const part of parts) {
                ...
                if (part.length > 0) {
                    ...
                    if (transformFunction) {
                        const transformedChunk = transformFunction(part, fallbackChunkId, streamState, strictOpenAiCompliance, gatewayRequest);
...
export async function handleTextResponse(response, responseTransformer) {
...
export async function handleNonStreamingMode(response, responseTransformer, strictOpenAiCompliance, gatewayRequestUrl, gatewayRequest, areSyncHooksAvailable) {
    ...
    const originalResponseBodyJson = isJsonParsingRequired ? await response.json() : null;
    let responseBodyJson = originalResponseBodyJson;
    if (responseTransformer) {
        responseBodyJson = responseTransformer(responseBodyJson, response.status, response.headers, strictOpenAiCompliance, gatewayRequestUrl, gatewayRequest);
    }
...
export function handleAudioResponse(response) {
    return new Response(response.body, response);
}
export function handleOctetStreamResponse(response) {
    return new Response(response.body, response);
}
export function handleImageResponse(response) {
    return new Response(response.body, response);
}
export function handleStreamingMode(response, proxyProvider, responseTransformer, requestURL, strictOpenAiCompliance, gatewayRequest) {
    ...
    if (proxyProvider === BEDROCK) {
        (async () => {
            for await (const chunk of readAWSStream(reader, responseTransformer, fallbackChunkId, strictOpenAiCompliance, gatewayRequest)) {
        ...
        (async () => {
            for await (const chunk of readStream(reader, splitPattern, responseTransformer, isSleepTimeRequired, fallbackChunkId, strictOpenAiCompliance, gatewayRequest)) {
                await writer.write(encoder.encode(chunk));
            }
            writer.close();
        })();
    ...
    const isVertexLlama = proxyProvider === GOOGLE_VERTEX_AI &&
...
export async function handleJSONToStreamResponse(response, provider, responseTransformerFunction) {
    const { readable, writable } = new TransformStream();
    const writer = writable.getWriter();
    const encoder = new TextEncoder();
    const responseJSON = await response.clone().json();
    if (Object.prototype.toString.call(responseTransformerFunction) ===
