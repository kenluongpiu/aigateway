import { AZURE_OPEN_AI, BEDROCK, WORKERS_AI, HEADER_KEYS, POWERED_BY, RESPONSE_HEADER_KEYS, RETRY_STATUS_CODES, GOOGLE_VERTEX_AI, OPEN_AI, AZURE_AI_INFERENCE, ANTHROPIC, CONTENT_TYPES, HUGGING_FACE, STABILITY_AI, SAGEMAKER, FIREWORKS_AI, CORTEX, } from '../globals';
 ...
 */
export function constructRequest(providerConfigMappedHeaders, provider, method, forwardHeaders, requestHeaders, fn, c) {
    const proxyHeaders = {};
    ...
    if (fn === 'proxy') {
        ...
        Object.keys(requestHeaders).forEach((key) => {
            if (!headersToIgnore.includes(key) &&
        ...
        if (proxyHeaders['accept-encoding']?.includes('br'))
            proxyHeaders['accept-encoding'] = proxyHeaders['accept-encoding']?.replace('br', '');
    }
    const baseHeaders = {
        'content-type': 'application/json',
    };
    let headers = {};
    Object.keys(providerConfigMappedHeaders).forEach((h) => {
        headers[h.toLowerCase()] = providerConfigMappedHeaders[h];
    });
    const forwardHeadersMap = {};
...
function getProxyPath(requestURL, proxyProvider, proxyEndpointPath, baseURL, providerOptions) {
...
export function convertHooksShorthand(hooksArr, type, hookType) {
    return hooksArr.map((hook) => {
        let hooksObject = {
            type: hookType,
 ...
 */
export async function tryPost(c, providerOption, requestBody, requestHeaders, fn, currentIndex, method = 'POST') {
    const overrideParams = providerOption?.overrideParams || {};
    let params = requestBody instanceof ReadableStream || requestBody instanceof FormData
        ? {}
    ...
    const hookSpan = hooksManager.createSpan(params, metadata, provider, isStreamingMode, [
    ...
    url =
        fn === 'proxy'
            ? getProxyPath(c.req.url, provider, c.req.url.indexOf('/v1/proxy') > -1 ? '/v1/proxy' : '/v1', baseUrl, providerOption)
            : `${baseUrl}${endpoint}`;
    let mappedResponse;
    let retryCount;
    ...
    let fetchOptions = {};
    const areSyncHooksAvailable = Boolean(hooksManager.getHooksToExecute(hookSpan, [
    ...
    if (brhResponse) {
        ...
        if (!providerConfig?.requestHandlers?.[fn]) {
            transformedRequestBody =
                method === 'POST'
                    ? transformToProviderRequest(provider, params, requestBody, fn, requestHeaders, providerOption)
                    : requestBody;
        }
    ...
    fetchOptions = constructRequest(headers, provider, method, forwardHeaders, requestHeaders, fn, c);
    ...
    async function createResponse(response, responseTransformer, isCacheHit, isResponseAlreadyMapped = false) {
        if (!isResponseAlreadyMapped) {
            ({ response: mappedResponse, originalResponseJson } =
                await responseHandler(response, isStreamingMode, provider, responseTransformer, url, isCacheHit, params, strictOpenAiCompliance, c.req.url, areSyncHooksAvailable));
        }
        updateResponseHeaders(mappedResponse, currentIndex, params, cacheStatus, retryCount ?? 0, requestHeaders[HEADER_KEYS.TRACE_ID] ?? '', provider);
        ...
        if (!mappedResponse.ok) {
            const errorObj = new Error(await mappedResponse.clone().text());
    ...
    // Cache Handler
    ({ cacheResponse, cacheStatus, cacheKey, createdAt } = await cacheHandler(c, providerOption, requestHeaders, fetchOptions, transformedRequestBody, hookSpan.id, fn));
    if (cacheResponse) {
    ...
    ({ mappedResponse, retryCount, createdAt, originalResponseJson } =
        await recursiveAfterRequestHookHandler(c, url, fetchOptions, providerOption, isStreamingMode, params, 0, fn, requestHeaders, hookSpan.id, strictOpenAiCompliance, requestBody));
    return createResponse(mappedResponse, undefined, false, true);
}
export async function tryTargetsRecursively(c, targetGroup, request, requestHeaders, fn, method, jsonPath, inheritedConfig = {}) {
    ...
    if (Object.keys(inheritedConfig).length === 0) {
        if (currentTarget.defaultInputGuardrails) {
            currentInheritedConfig.defaultInputGuardrails = [
                ...convertHooksShorthand(currentTarget.defaultInputGuardrails, 'input', HookType.GUARDRAIL),
            ];
        }
        if (currentTarget.defaultOutputGuardrails) {
            currentInheritedConfig.defaultOutputGuardrails = [
                ...convertHooksShorthand(currentTarget.defaultOutputGuardrails, 'output', HookType.GUARDRAIL),
            ];
        }
    }
    ...
    if (currentTarget.inputGuardrails) {
        currentTarget.beforeRequestHooks = [
            ...(currentTarget.beforeRequestHooks || []),
            ...convertHooksShorthand(currentTarget.inputGuardrails, 'input', HookType.GUARDRAIL),
        ];
    }
    if (currentTarget.outputGuardrails) {
        currentTarget.afterRequestHooks = [
            ...(currentTarget.afterRequestHooks || []),
            ...convertHooksShorthand(currentTarget.outputGuardrails, 'output', HookType.GUARDRAIL),
        ];
    }
    if (currentTarget.inputMutators) {
        currentTarget.beforeRequestHooks = [
            ...(currentTarget.beforeRequestHooks || []),
            ...convertHooksShorthand(currentTarget.inputMutators, 'input', HookType.MUTATOR),
        ];
    }
    if (currentTarget.outputMutators) {
        currentTarget.afterRequestHooks = [
            ...(currentTarget.afterRequestHooks || []),
            ...convertHooksShorthand(currentTarget.outputMutators, 'output', HookType.MUTATOR),
        ];
    }
    ...
    switch (strategyMode) {
        case StrategyModes.FALLBACK:
            for (const [index, target] of currentTarget.targets.entries()) {
                response = await tryTargetsRecursively(c, target, request, requestHeaders, fn, method, `${currentJsonPath}.targets[${index}]`, currentInheritedConfig);
        ...
        case StrategyModes.LOADBALANCE:
            currentTarget.targets.forEach((t) => {
                if (t.weight === undefined) {
                    t.weight = 1;
                }
            });
            let totalWeight = currentTarget.targets.reduce((sum, provider) => sum + provider.weight, 0);
            let randomWeight = Math.random() * totalWeight;
            for (const [index, provider] of currentTarget.targets.entries()) {
                if (randomWeight < provider.weight) {
                    currentJsonPath = currentJsonPath + `.targets[${index}]`;
                    response = await tryTargetsRecursively(c, provider, request, requestHeaders, fn, method, currentJsonPath, currentInheritedConfig);
                    break;
                }
                randomWeight -= provider.weight;
            }
        ...
        case StrategyModes.CONDITIONAL: {
            ...
            catch (conditionalRouter) {
                throw new RouterError(conditionalRouter.message);
            }
            response = await tryTargetsRecursively(c, finalTarget, request, requestHeaders, fn, method, `${currentJsonPath}.targets[${finalTarget.index}]`, currentInheritedConfig);
            break;
        }
        case StrategyModes.SINGLE:
            response = await tryTargetsRecursively(c, currentTarget.targets[0], request, requestHeaders, fn, method, `${currentJsonPath}.targets[0]`, currentInheritedConfig);
            break;
        default:
            try {
                response = await tryPost(c, currentTarget, request, requestHeaders, fn, currentJsonPath, method);
            }
            catch (error) {
 ...
 */
export function updateResponseHeaders(response, currentIndex, params, cacheStatus, retryAttempt, traceId, provider) {
...
export function constructConfigFromRequestHeaders(requestHeaders) {
    const azureConfig = {
    ...
    const awsConfig = {
        ...
        awsServerSideEncryptionKMSKeyId: requestHeaders[`x-${POWERED_BY}-amz-server-side-encryption-aws-kms-key-id`],
    };
    const sagemakerConfig = {
        amznSagemakerCustomAttributes: requestHeaders[`x-${POWERED_BY}-amzn-sagemaker-custom-attributes`],
        ...
        amznSagemakerTargetContainerHostname: requestHeaders[`x-${POWERED_BY}-amzn-sagemaker-target-container-hostname`],
        amznSagemakerInferenceId: requestHeaders[`x-${POWERED_BY}-amzn-sagemaker-inference-id`],
        amznSagemakerEnableExplanations: requestHeaders[`x-${POWERED_BY}-amzn-sagemaker-enable-explanations`],
        amznSagemakerInferenceComponent: requestHeaders[`x-${POWERED_BY}-amzn-sagemaker-inference-component`],
    ...
    if (vertexServiceAccountJson) {
        try {
            vertexConfig.vertexServiceAccountJson = JSON.parse(vertexServiceAccountJson);
        }
        catch (e) {
...
export async function recursiveAfterRequestHookHandler(c, url, options, providerOption, isStreamingMode, gatewayParams, retryAttemptsMade, fn, requestHeaders, hookSpanId, strictOpenAiCompliance, requestBody) {
    let response, retryCount, createdAt, executionTime, retrySkipped;
    const requestTimeout = Number(requestHeaders[HEADER_KEYS.REQUEST_TIMEOUT]) ||
    ...
    if (requestHandlers && requestHandlers[fn]) {
        requestHandler = () => requestHandlers[fn]({
            c,
            providerOptions: providerOption,
    ...
    ({
        ...
        createdAt,
        skip: retrySkipped,
    } = await retryRequest(url, options, retry?.attempts || 0, retry?.onStatusCodes || [], requestTimeout || null, requestHandler, retry?.useRetryAfterHeader || false));
    const hooksManager = c.get('hooksManager');
    const hookSpan = hooksManager.getSpan(hookSpanId);
    ...
    const { response: mappedResponse, responseJson: mappedResponseJson, originalResponseJson, } = await responseHandler(response, isStreamingMode, providerOption, fn, url, false, gatewayParams, strictOpenAiCompliance, c.req.url, areSyncHooksAvailable);
    const arhResponse = await afterRequestHookHandler(c, mappedResponse, mappedResponseJson, hookSpanId, retryAttemptsMade);
    const remainingRetryCount = (retry?.attempts || 0) - (retryCount || 0) - retryAttemptsMade;
    const isRetriableStatusCode = retry?.onStatusCodes?.includes(arhResponse.status);
    if (remainingRetryCount > 0 && !retrySkipped && isRetriableStatusCode) {
        return recursiveAfterRequestHookHandler(c, url, options, providerOption, isStreamingMode, gatewayParams, (retryCount || 0) + 1 + retryAttemptsMade, fn, requestHeaders, hookSpanId, strictOpenAiCompliance);
    }
 ...
 */
function getCacheOptions(cacheConfig) {
...
async function cacheHandler(c, providerOption, requestHeaders, fetchOptions, transformedRequestBody, hookSpanId, fn) {
    if ([
        'uploadFile',
        'listFiles',
        'retrieveFile',
    ...
    let cacheMode, cacheMaxAge, cacheStatus;
    ({ cacheMode, cacheMaxAge, cacheStatus } = getCacheOptions(providerOption.cache));
    if (getFromCacheFunction && cacheMode) {
        [cacheResponse, cacheStatus, cacheKey] = await getFromCacheFunction(env(c), { ...requestHeaders, ...fetchOptions.headers }, transformedRequestBody, fn, cacheIdentifier, cacheMode, cacheMaxAge);
    }
    const hooksManager = c.get('hooksManager');
    const span = hooksManager.getSpan(hookSpanId);
    const results = span.getHooksResult();
    const failedBeforeRequestHooks = results.beforeRequestHooksResult?.filter((h) => !h.verdict);
...
export async function beforeRequestHookHandler(c, hookSpanId) {
    let span;
    let isTransformed = false;
    try {
        const start = new Date();
        const hooksManager = c.get('hooksManager');
        const hooksResult = await hooksManager.executeHooks(hookSpanId, ['syncBeforeRequestHook'], {
            env: env(c),
            getFromCacheByKey: c.get('getFromCacheByKey'),
            putInCacheWithValue: c.get('putInCacheWithValue'),
        });
        span = hooksManager.getSpan(hookSpanId);
