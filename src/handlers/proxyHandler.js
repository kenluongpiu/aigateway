...
async function getRequestData(request, contentType) {
    let finalRequest;
...
export async function proxyHandler(c) {
    try {
        ...
        const tryTargetsResponse = await tryTargetsRecursively(c, camelCaseConfig, request, requestHeaders, 'proxy', c.req.method, 'config');
