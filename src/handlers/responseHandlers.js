 ...
 */
export async function responseHandler(response, streamingMode, provider, responseTransformer, requestURL, isCacheHit = false, gatewayRequest, strictOpenAiCompliance, gatewayRequestUrl, areSyncHooksAvailable) {
    ...
    if (streamingMode &&
        ...
        isCacheHit &&
        responseTransformerFunction) {
        const streamingResponse = await handleJSONToStreamResponse(response, provider, responseTransformerFunction);
        return { response: streamingResponse, responseJson: null };
    }
    if (streamingMode && isSuccessStatusCode) {
        return {
            response: handleStreamingMode(response, provider, responseTransformerFunction, requestURL, strictOpenAiCompliance, gatewayRequest),
            responseJson: null,
        };
    }
    ...
    if (responseContentType?.startsWith(CONTENT_TYPES.PLAIN_TEXT) ||
    ...
    const nonStreamingResponse = await handleNonStreamingMode(response, responseTransformerFunction, strictOpenAiCompliance, gatewayRequestUrl, gatewayRequest, areSyncHooksAvailable);
    return {
...
function createHookResponse(baseResponse, responseData, hooksResult, options = {}) {
    const responseBody = {
        ...(options.forceError
            ? {
                error: {
...
export async function afterRequestHookHandler(c, response, responseJSON, hookSpanId, retryAttemptsMade) {
    try {
        const hooksManager = c.get('hooksManager');
        hooksManager.setSpanContextResponse(hookSpanId, responseJSON, response.status);
        if (retryAttemptsMade > 0) {
            hooksManager.getSpan(hookSpanId).resetHookResult('afterRequestHook');
        }
        const { shouldDeny } = await hooksManager.executeHooks(hookSpanId, ['syncAfterRequestHook'], {
            env: env(c),
        ...
        const failedBeforeRequestHooks = hooksResult.beforeRequestHooksResult.filter((h) => !h.verdict);
        const failedAfterRequestHooks = hooksResult.afterRequestHooksResult.filter((h) => !h.verdict);
        if (!responseJSON) {
