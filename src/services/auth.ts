...
class AuthenticationService {
  ...
  constructor(env?: any) {
    ...
    this.jwtSecret = env?.JWT_SECRET || process.env.JWT_SECRET || 'fallback-secret-key';
    this.jwtExpiresIn = env?.JWT_EXPIRES_IN || process.env.JWT_EXPIRES_IN || '14d';
  }
  /**
   * User Registration
   */
   ...
   * Admin Login
   */
  public async adminLogin(c: Context): Promise<Response> {
    try {
      const body = await c.req.json() as LoginRequest;
   ...
   * Get User Profile
   */
  public async getProfile(c: Context): Promise<Response> {
    try {
   ...
   * Create default virtual key for new user
   */
  private async createDefaultVirtualKey(customerId: number, email: string): Promise<void> {
    try {
      const crypto = await import('crypto');
      const bcrypt = await import('bcryptjs');
