...
export const transformUsingProviderConfig = (providerConfig, params, providerOptions) => {
 ...
 */
const transformToProviderRequestJSON = (provider, params, fn, providerOptions) => {
...
const transformToProviderRequestFormData = (provider, params, fn) => {
    let providerConfig = ProviderConfigs[provider];
    if (providerConfig.getConfig) {
        providerConfig = providerConfig.getConfig(params)[fn];
    }
...
const transformToProviderRequestReadableStream = (provider, requestBody, requestHeaders, fn) => {
 ...
 */
export const transformToProviderRequest = (provider, params, requestBody, fn, requestHeaders, providerOptions) => {
    // this returns a ReadableStream
    if (fn === 'uploadFile') {
        return transformToProviderRequestReadableStream(provider, requestBody, requestHeaders, fn);
    }
    ...
    if (providerAPIConfig.transformToFormData &&
        providerAPIConfig.transformToFormData({ gatewayRequestBody: params }))
        return transformToProviderRequestFormData(provider, params, fn);
