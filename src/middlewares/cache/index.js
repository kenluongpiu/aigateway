...
// Cache Handling
export const getFromCache = async (env, requestHeaders, requestBody, url, organisationId, cacheMode, cacheMaxAge) => {
...
export const putInCache = async (env, requestHeaders, requestBody, responseBody, url, organisationId, cacheMode, cacheMaxAge) => {
    ...
    return async (c, next) => {
        ...
        if (requestOptions &&
            ...
            if (requestOptions.cacheMode === 'simple') {
                await putInCache(null, null, requestOptions.requestParams, await requestOptions.response.json(), requestOptions.providerOptions.rubeusURL, '', null, null);
            }
        }
    };
};
