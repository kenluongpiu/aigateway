import { z } from 'zod';
import { OLLAMA, VALID_PROVIDERS, GOOGLE_VERTEX_AI, TRITON, } from '../../../globals';
export const configSchema = z
    ...
    strategy: z
        ...
        mode: z
            ...
            message: "Invalid 'mode' value. Must be one of: single, loadbalance, fallback, conditional",
        }),
    ...
    input_guardrails: z
        .union([
        z.array(z.string()),
        z.array(z.object({}).catchall(z.any()) // Allows any object structure
        ),
    ])
        .optional(),
    output_guardrails: z
    ...
    const isVertexAIProvider = value.provider === GOOGLE_VERTEX_AI &&
        value.vertex_region &&
    ...
    message: "Invalid configuration. It must have either 'provider' and 'api_key', or 'strategy' and 'targets', or 'cache', or 'retry', or 'request_timeout'",
})
    .refine((value) => {
    ...
    const hasGoogleVertexAIFields = (value.vertex_project_id && value.vertex_region) ||
