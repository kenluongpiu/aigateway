...
class AuthService {
   ...
   */
  public async jwtAuth(c: Context, next: Next): Promise<Response | void> {
    try {
   ...
   */
  public async virtualKeyAuth(c: Context, next: Next): Promise<Response | void> {
    try {
   ...
   */
  public async adminAuth(c: Context, next: Next): Promise<Response | void> {
   ...
   */
  private async getUserById(userId: number, role: 'customer' | 'admin') {
    try {
      if (role === 'admin') {
        const result = await this.db.query(
          'SELECT id, username as email, full_name, role FROM admin_users WHERE id = $1 AND is_active = true',
          [userId]
        );
        ...
        const result = await this.db.query(
          'SELECT id, email, full_name, plan_id FROM customers WHERE id = $1 AND status = $2',
          [userId, 'active']
        );
   ...
   */
  private async getVirtualKey(apiKey: string) {
    try {
   ...
   */
  private async updateKeyLastUsed(keyId: number) {
    try {
      await this.db.query(
        'UPDATE virtual_keys SET last_used = CURRENT_TIMESTAMP WHERE id = $1',
        [keyId]
      );
    } catch (error) {
