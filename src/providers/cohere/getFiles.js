...
const transformDatasetUploadStatus = (status) => {
...
const transformCohereDataset = (dataset) => {
    return {
        id: dataset.id,
        object: 'file',
        bytes: 0,
...
export const CohereGetFileResponseTransform = (response, responseStatus) => {
...
export const CohereGetFilesResponseTransform = (response, responseStatus) => {
    if (responseStatus !== 200 && 'message' in response) {
        return CohereErrorResponseTransform(response);
    }
