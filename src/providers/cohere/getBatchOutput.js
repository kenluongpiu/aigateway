...
export const CohereGetBatchOutputHandler = async ({ c, providerOptions, requestURL, }) => {
    try {
        ...
        const batchDetails = await retrieveBatchResponse.json();
        const outputFileId = batchDetails.output_dataset_id;
        // get the file details
        const retrieveFileResponse = await fetch(`https://api.cohere.ai/v1/datasets/${outputFileId}`, {
            method: 'GET',
            headers,
        });
        const retrieveFileResponseJson = await retrieveFileResponse.json();
            ...
            start: async (controller) => {
                ...
                for (const filePart of fileParts) {
                    ...
                    decoder.on('data', (data) => {
                        controller.enqueue(encoder.encode(JSON.stringify(CohereEmbedResponseTransformBatch(JSON.parse(data)))));
