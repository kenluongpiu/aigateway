...
export const OpenAIChatCompleteConfig = {
    model: {
        param: 'model',
        required: true,
...
export const OpenAIChatCompleteResponseTransform = (response, responseStatus) => {
 ...
 */
export const OpenAIChatCompleteJSONToStreamResponseTransform = (response, provider) => {
    const streamChunkArray = [];
    const { id, model, system_fingerprint, choices, citations } = response;
    const { prompt_tokens, completion_tokens, cache_read_input_tokens, cache_creation_input_tokens, num_search_queries, } = response.usage || {};
    let total_tokens;
    if (prompt_tokens && completion_tokens)
        total_tokens = prompt_tokens + completion_tokens;
    const shouldSendCacheUsage = provider === ANTHROPIC &&
