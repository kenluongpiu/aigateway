...
export const BedrockRetrieveFileContentRequestHandler = async ({ c, providerOptions, requestURL, }) => {
    try {
        ...
        let responseStream;
        if (response?.body) {
            responseStream = response?.body?.pipeThrough(getOctetStreamToOctetStreamTransformer(getRowTransform()));
        }
        else {
            throw new Error('Failed to parse and transform file content, please verify that the file is a valid jsonl file used for batching or fine-tuning');
        }
        // return the response
...
export const BedrockRetrieveFileContentResponseTransform = (response) => {
    return response;
};
