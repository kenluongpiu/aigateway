...
export const generateAWSHeaders = async (body, headers, url, method, awsService, awsRegion, awsAccessKeyID, awsSecretAccessKey, awsSessionToken) => {
...
export const transformInferenceConfig = (params) => {
...
export const transformAdditionalModelRequestFields = (params) => {
    const additionalModelRequestFields = params.additionalModelRequestFields ||
...
export const transformAnthropicAdditionalModelRequestFields = (params) => {
    const additionalModelRequestFields = params.additionalModelRequestFields ||
        params.additional_model_request_fields ||
        {};
    if (params['top_k']) {
...
export const transformCohereAdditionalModelRequestFields = (params) => {
    const additionalModelRequestFields = params.additionalModelRequestFields ||
        params.additional_model_request_fields ||
        {};
    if (params['top_k']) {
...
export const transformAI21AdditionalModelRequestFields = (params) => {
    const additionalModelRequestFields = params.additionalModelRequestFields ||
        params.additional_model_request_fields ||
        {};
    if (params['top_k']) {
...
export async function getAssumedRoleCredentials(c, awsRoleArn, awsExternalId, awsRegion, creds) {
    ...
    if (creds) {
        ...
        const { AWS_ASSUME_ROLE_ACCESS_KEY_ID, AWS_ASSUME_ROLE_SECRET_ACCESS_KEY } = env(c);
...
export const bedrockFinetuneToOpenAI = (finetune) => {
    let status = 'running';
    ...
    return {
        ...
        fine_tuned_model: finetune.outputModelArn ||
...
export async function providerAssumedRoleCredentials(c, providerOptions) {
    try {
        // Assume the role in the source account
        const sourceRoleCredentials = await getAssumedRoleCredentials(c, env(c).AWS_ASSUME_ROLE_SOURCE_ARN, // Role ARN in the source account
        ...
        const { accessKeyId, secretAccessKey, sessionToken } = (await getAssumedRoleCredentials(c, providerOptions.awsRoleArn || '', providerOptions.awsExternalId || '', providerOptions.awsRegion || '', {
...
export const populateHyperParameters = (value) => {
