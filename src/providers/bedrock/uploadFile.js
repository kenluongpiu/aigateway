...
import { BedrockUploadFileTransformerConfig, transformFinetuneDatasetLine, tryChatToTextTransformation, } from './uploadFileUtils';
...
class AwsMultipartUploadHandler {
    ...
    c;
    contentLength = 0;
    constructor(region = 'us-east-1', bucket = '', objectKey = '', providerOptions, c) {
        this.region = region;
        this.bucket = bucket;
        this.objectKey = objectKey;
        this.url = new URL(`https://${bucket}.s3.${region}.amazonaws.com/${objectKey}?uploads`);
    ...
    async chunkAndUploadStream(awsBedrockModel, requestBody, requestHeaders, purpose, modelType) {
        ...
        while (true) {
            ...
            // 1MB chunk size
            if (currentChunk.length < 1000000 && !done)
                continue;
            while (currentChunk.length > 0) {
                if (isParsingHeaders) {
                    const headersEndIndex = currentChunk.indexOf('\r\n\r\n');
                    const boundaryEndIndex = currentChunk.indexOf(boundary) + boundary.length + 2;
                    ...
                    if (isFileContent) {
                        ...
                        if (fileExtension !== 'jsonl') {
                            throw new Error('Invalid file extension, only jsonl files are supported');
                        }
                    }
                ...
                if (isFileContent) {
                    ...
                    [currentChunk, uploadLength] =
                        await transformAndUploadFileContentParts(content, currentChunk, providerConfig, this, partNumber, purpose ?? 'batch', modelType ?? 'chat');
    ...
    async uploadPart(partNumber, partData) {
        const method = 'PUT';
        const partUrl = new URL(`https://${this.bucket}.s3.${this.region}.amazonaws.com/${this.objectKey}?partNumber=${partNumber}&uploadId=${this.uploadId}`);
        const headers = await BedrockAPIConfig.headers({
            c: this.c,
            providerOptions: this.providerOptions,
            fn: 'uploadFile',
        ...
        if (!eTag)
            throw new Error(`unable to upload to s3, please try again`);
        this.parts.push({ PartNumber: partNumber, ETag: eTag });
    }
    async completeMultipartUpload() {
        const method = 'POST';
        const completeUrl = new URL(`https://${this.bucket}.s3.${this.region}.amazonaws.com/${this.objectKey}?uploadId=${this.uploadId}`);
...
const transformAndUploadFileContentParts = async (chunk, buffer, providerConfig, handler, partNumber, purpose, modelType) => {
    ...
    for (let i = 0; i < numberOfLines; i++) {
        ...
        try {
            ...
            if (purpose === 'fine-tune') {
                const transformedLine = modelType === 'text'
            ...
            if (i !== numberOfLines - 1) {
                throw new Error('Your file contains invalid json lines, or empty lines, please fix them and try again: ' +
                    '\n failing line: ' +
                    line +
                    '\n failing chunk: ' +
                    chunk);
...
const getProviderConfig = (modelSlug) => {
    let provider = '';
    if (modelSlug.includes('llama2'))
        provider = 'llama2';
    else if (modelSlug.includes('llama3'))
        provider = 'llama3';
    else if (modelSlug.includes('titan'))
        provider = 'titan';
    else if (modelSlug.includes('mistral'))
        provider = 'mistral';
    else if (modelSlug.includes('anthropic'))
        provider = 'anthropic';
    else if (modelSlug.includes('ai21'))
        provider = 'ai21';
    else if (modelSlug.includes('cohere'))
        provider = 'cohere';
    else
        throw new Error('Invalid model slug');
    return BedrockUploadFileTransformerConfig[provider];
};
export const BedrockUploadFileRequestHandler = async ({ providerOptions: providerOptions, requestBody, requestHeaders, c, }) => {
    try {
        ...
        if (!awsS3Bucket || !awsBedrockModel) {
            return new Response(JSON.stringify({
                status: 'failure',
                message: 'Please make sure you have x-portkey-aws-s3-bucket and x-portkey-aws-bedrock-model headers provided.',
            }), {
                status: 400,
                headers: {
        ...
        const handler = new AwsMultipartUploadHandler(awsRegion, awsS3Bucket, awsS3ObjectKey, providerOptions, c);
        ...
        await handler.chunkAndUploadStream(awsBedrockModel, requestBody, requestHeaders, purpose, modelType);
...
export const BedrockUploadFileResponseTransform = (response) => {
    return response;
};
