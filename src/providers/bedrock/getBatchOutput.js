...
export const BedrockGetBatchOutputRequestHandler = async ({ c, providerOptions, requestURL, }) => {
    try {
        ...
        let responseStream;
        if (s3FileResponse.headers.get('content-type')?.includes('octet-stream') &&
            s3FileResponse?.body) {
            responseStream = s3FileResponse?.body?.pipeThrough(getOctetStreamToOctetStreamTransformer(getRowTransform(awsModelProvider)));
...
export const BedrockGetBatchOutputResponseTransform = (response) => {
    return response;
};
