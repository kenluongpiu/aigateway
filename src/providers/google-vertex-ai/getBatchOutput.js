...
import { GoogleChatCompleteResponseTransform, VertexAnthropicChatCompleteResponseTransform, VertexLlamaChatCompleteResponseTransform, } from './chatComplete';
...
const getOpenAIBatchRow = ({ row, batchId, transform, }) => {
...
export const BatchOutputRequestHandler = async ({ requestURL, providerOptions, c, requestBody, }) => {
    ...
    try {
        ...
        const errorMessage = error.message || 'Failed to retrieve batch output';
    ...
    let responseTransform = responseTransforms[provider] ||
