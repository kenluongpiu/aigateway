...
import { SYSTEM_INSTRUCTION_DISABLED_MODELS, transformOpenAIRoleToGoogleRole, transformToolChoiceForGemini, } from '../google/chatComplete';
import { generateErrorResponse, generateInvalidProviderResponseError, } from '../utils';
import { transformGenerationConfig } from './transformGenerationConfig';
import { getMimeType, recursivelyDeleteUnsupportedParameters, transformVertexLogprobs, } from './utils';
export const buildGoogleSearchRetrievalTool = (tool) => {
...
export const VertexGoogleChatCompleteConfig = {
...
export const VertexAnthropicChatCompleteConfig = {
...
export const VertexLlamaChatCompleteConfig = {
    model: {
        param: 'model',
        required: true,
        default: 'meta/llama-3.1-405b-instruct-maas',
        transform: (params) => {
            return (params.model?.replace('meta.', 'meta/') ||
    ...
    const dataChunk = {
        ...
        choices: parsedChunk.candidates?.map((generation, index) => {
            ...
            if (generation.content?.parts[0]?.text) {
                ...
                let content = strictOpenAiCompliance && streamState.containsChainOfThoughtMessage
                    ? ''
...
export const VertexAnthropicChatCompleteResponseTransform = (response, responseStatus, _responseHeaders, strictOpenAiCompliance) => {
    if (responseStatus !== 200) {
...
export const VertexAnthropicChatCompleteStreamChunkTransform = (responseChunk, fallbackId, streamState, strictOpenAiCompliance) => {
...
export const VertexLlamaChatCompleteResponseTransform = (response, responseStatus) => {
    if (responseStatus !== 200 &&
        Array.isArray(response) &&
        response.length > 0 &&
        'error' in response[0]) {
...
export const VertexLlamaChatCompleteStreamChunkTransform = (responseChunk, fallbackId) => {
    let chunk = responseChunk.trim();
    chunk = chunk.replace(/^data: /, '');
    chunk = chunk.trim();
    const parsedChunk = JSON.parse(chunk);
