...
export const getAccessToken = async (c, serviceAccountInfo) => {
    try {
...
export const getModelAndProvider = (modelString) => {
    let provider = 'google';
    let model = modelString;
    const modelStringParts = modelString.split('.');
    if (modelStringParts.length > 1 &&
...
export const GoogleErrorResponseTransform = (response, provider = GOOGLE_VERTEX_AI) => {
...
const getDefFromRef = (ref) => {
    const refParts = ref.split('/');
    return refParts.at(-1);
};
const getRefParts = (spec, ref) => {
    return spec?.[ref];
};
export const derefer = (spec, defs = null) => {
    const original = { ...spec };
...
export const recursivelyDeleteUnsupportedParameters = (obj) => {
...
export const GoogleResponseHandler = (response, status) => {
    if (status !== 200) {
...
export const googleBatchStatusToOpenAI = (status) => {
    switch (status) {
...
export const googleFinetuneStatusToOpenAI = (status) => {
    switch (status) {
        case 'JOB_STATE_CANCELLED':
        case 'JOB_STATE_CANCELLING':
        case 'JOB_STATE_EXPIRED':
...
export const GoogleToOpenAIBatch = (response) => {
    const jobId = response.name.split('/').at(-1);
    const total = Object.values(response.completionsStats ?? {}).reduce((acc, current) => acc + Number.parseInt(current), 0);
...
export const fetchGoogleCustomEndpoint = async ({ authorization, method, url, body, }) => {
...
export const transformVertexLogprobs = (generation) => {
...
const populateHyperparameters = (value) => {
...
export const transformVertexFinetune = (params) => {
    const parameterSpec = {
...
export const getBucketAndFile = (uri) => {
    if (!uri)
        return { bucket: '', file: '' };
...
export const GoogleToOpenAIFinetune = (response) => {
    return {
        ...
        hyperparameters: {
            ...
            learning_rate_multiplier: response.supervisedTuningSpec.hyperParameters.learningRateMultiplier,
        ...
        trained_tokens: response.tuningDataStats?.supervisedTuningDataStats
