...
export const GoogleRetrieveFileRequestHandler = async ({ requestURL, providerOptions, }) => {
    const fileId = requestURL.split('/').pop();
    const { bucket, file } = getBucketAndFile(fileId ?? '');
    const headers = await GoogleApiConfig.headers({ providerOptions });
...
export const GoogleRetrieveFileResponseTransform = (response) => {
    return response;
};
