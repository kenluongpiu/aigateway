...
const getApiVersion = (provider, inputModel) => {
    if (provider === 'meta')
        return 'v1beta1';
    return 'v1';
};
const getProjectRoute = (providerOptions, inputModel) => {
    const { vertexProjectId: inputProjectId, vertexRegion, vertexServiceAccountJson, } = providerOptions;
    ...
    let routeVersion = getApiVersion(provider, inputModel);
...
export const GoogleApiConfig = {
    ...
    getEndpoint: ({ fn, gatewayRequestBodyJSON: gatewayRequestBody, providerOptions, gatewayRequestURL, }) => {
